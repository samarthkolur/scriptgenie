...
1. **🔴 Still open — rotate the leaked key.** The credential was removed from `prompt.txt` and never entered git history, but it stays valid until revoked at the provider. It currently lives in `apps/api/.env`, which is gitignored and untracked — correct placement, and the reason `pnpm check:secrets` reports one finding on a working tree that has it. That finding is the unrotated key, not a leak into the repository.
